...
# Self-locate: this script lives in articles-in-progress/types-of-real-estate-leads/,
# project root is two levels up.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
PAYLOAD_FILE="$SCRIPT_DIR/wp-payload-types-of-real-estate-leads.json"

# 1. Validate .env exists at the project root
if [ ! -f "$PROJECT_ROOT/.env" ]; then
  echo "❌ No .env found at $PROJECT_ROOT/.env"
  echo "   Create it with WP_URL, WP_USER, WP_APP_PASSWORD (App Password quoted)."
  exit 1
fi
