...
PAYLOAD_FILE="$SCRIPT_DIR/wp-payload-search-engine-optimization-services-for-real-estate.json"

# 1. Validate .env exists at the project root
if [ ! -f "$PROJECT_ROOT/.env" ]; then
  echo "❌ No .env found at $PROJECT_ROOT/.env"
  echo "   Create it with WP_URL, WP_USER, WP_APP_PASSWORD (see SETUP-wordpress.md)."
  exit 1
fi
...
case "$HTTP_CODE" in
  201)
    ...
    echo
    echo "   Before publishing: 4 [SCREENSHOT:] placeholders need images."
    echo "   Copy each [PROMPT FOR CHATGPT IMAGE GENERATION:] block into ChatGPT,"
    echo "   generate the image, and replace BOTH blocks with the real image in the editor."
    ;;
  401)
