...
<#
  ...
  - No publica: status = "draft" (viene en el payload).
...
$PayloadFile = Join-Path $ScriptDir "wp-payload-where-to-find-foreclosure-listings.json"
...
try {
    ...
    Write-Host "     Pendiente: generar las 2 imagenes (bloques PROMPT) y reemplazar {{CLIENT_OFFER_URL}} en la CTA." -ForegroundColor Yellow
}
catch {
    $code = $null
